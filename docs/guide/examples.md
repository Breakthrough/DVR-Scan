...
The following parameters directly affect the "sensitivity" of the detection algorithm, and may need to be adjusted for some video footage to achieve desirable results.
 ...
 - `-roi` / `--rectangle-of-interest`:  If specified, scans only in the selected rectangular area.  Can select with mouse, or by specifying a bounding box of the form `[x y w h]` (e.g. `-roi` allows specifying the bounding box with mouse, and `-roi 100 110 50 75` specify a bounding box starting at (100,110) with a width and height of 50x75 pixels).  Selection is made with the mouse by clicking-and-dragging, followed by hitting the enter/return key when the selection is finalized.
