 ...
 * <b><pre>-mo mask.avi, --mask-output mask.avi</pre></b> Save a video containing the calculated motion mask on each frame. Useful for tuning motion detection parameters. Requires `.avi` extension.
 ...
 * <b><pre>-roi, --region-of-interest</pre></b> Show a pop-up window to select a region of interest using the mouse. The first frame will be displayed.

 * <b><pre>-roi x0, y0 w, h</pre></b> Rectangle specified as top-left corner and size. For example, `-roi 50,75, 100,150` specifies a 100x150 rectangle with the top left corner at coordinates (50,75). (0,0) is the top-left corner of the video.

 * <b><pre>-roi width,height</pre></b> Same as `-roi` but shrinks the window to fit within (width x height). Useful for processing videos larger than the monitor resolution.
