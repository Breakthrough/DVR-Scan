...
class ScanWindow:
    def __init__(
        ...
        self._elapsed_label = tk.Label(self._root, text="Elapsed:\n00:00")
        self._remaining_label = tk.Label(self._root, text="Remaining:\n??:??")
        ...
        self._events_label = tk.Label(self._root, text="Events Found: 0")
        self._processed_label = tk.Label(self._root, text="Processed:\n0 / ? frames")
        self._speed_label = tk.Label(self._root, text="Rate:\n??? FPS")
        ...
        self._root.rowconfigure(4, weight=32)
        self._root.rowconfigure(5, weight=2)
        self._root.minsize(width=self._root.winfo_reqwidth(), height=self._root.winfo_reqheight())

        self._events_label.grid(row=0, column=0, columnspan=2, sticky=tk.EW, pady=(padding, 0))
        self._progress_bar.grid(sticky=tk.NSEW, row=1, columnspan=2, pady=padding, padx=padding)
        self._elapsed_label.grid(row=2, column=0, sticky=tk.NSEW)
        self._remaining_label.grid(row=2, column=1, sticky=tk.NSEW)
        self._processed_label.grid(row=3, column=0, sticky=tk.NSEW)
        self._speed_label.grid(row=3, column=1, sticky=tk.NSEW)
        self._stop_button.grid(row=5, column=0, pady=padding, sticky="nsew", padx=padding)
        self._close_button.grid(row=5, column=1, pady=padding, sticky="nsew", padx=padding)
    ...
    def _update(self):
        if self._scan_finished.is_set():
            ...
            self._elapsed_label["text"] = f"Elapsed:\n{self._elapsed}"
            self._remaining_label.grid_forget()
            self._elapsed_label.grid_forget()
            self._elapsed_label.grid(row=2, column=0, columnspan=2, sticky=tk.NSEW)
            self._stop_button["state"] = tk.DISABLED
            self._close_button["state"] = tk.NORMAL
            self._processed_label["text"] = (
                f"Processed:\n{self._expected_num_frames} / {self._expected_num_frames} frames"
            )
            self._speed_label["text"] = f"Rate:\n{self._rate} FPS"
            return
        ...
        self._elapsed_label["text"] = f"Elapsed:\n{self._elapsed}"
        self._remaining_label["text"] = f"Remaining:\n{self._remaining}"
        self._processed_label["text"] = (
            f"Processed:\n{self._frames_processed} / {self._expected_num_frames} frames"
        )
        self._speed_label["text"] = f"Rate:\n{self._rate} FPS"
        self._root.after(UI_UPDATE_RATE_MS, self._update)
