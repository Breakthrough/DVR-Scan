...
KEYBIND_DOWNSCALE_INC = f"{ACCELERATOR_KEY}+Minus"
KEYBIND_DOWNSCALE_DEC = f"{ACCELERATOR_KEY}+Plus"
...
class RegionEditor:
    def __init__(
        ...
        self._source_frame: np.nd = frame.copy()  # Frame before downscaling
    ...
    def run(self) -> bool:
        ...
        def clamp_scale(val: str):
            new_val = round(float(val))
            if self._scale != new_val:
                self._scale = new_val
                # Disable resize since we're using the mouse input here.
        ...
        self._scale_widget.grid(row=3, column=0, sticky="sew", padx=16.0)
        ...
        self._root.focus()
        self._root.mainloop()
        dvr_scan._GLOBAL_APP_HANDLE = None
    ...
    def _show_help(self):
        if self._controls_window is None:
            ...
            ttk.Label(
                viewport,
                text=f"{ACCELERATOR_KEY} + Scroll Mouse\nKeyboard: "
                f"{KEYBIND_DOWNSCALE_INC}/{KEYBIND_DOWNSCALE_DEC}",
                justify=tk.RIGHT,
            ).grid(row=0, column=1, sticky="e")
            ttk.Label(viewport, text="Mouse4 (-)    Mouse5 (+)").grid(row=1, column=1, sticky="e")

            ttk.Label(viewport, text="").grid(row=2, column=0, sticky="w")
