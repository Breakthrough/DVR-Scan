...
from dvr_scan.scanner.roi import RegionsOfInterest
...
@dataclass
class MotionEvent:
    """Event generated by main thread after each motion event is finished."""
    start: FrameTimecode
    end: FrameTimecode
...
class DetectionMask:
    ...
    def __init__(self, rectangles: List[Tuple[int, int, int, int]]):
        ...
        if len(rectangles) == 1:
            ...
            # TODO: Generate a 2d numpy array that can be used as a mask.
            raise NotImplementedError()

    def get_bounds(self):
        raise NotImplementedError()

    def sum(frame: numpy.ndarray, origin=(0, 0)):
...
class ScanContext:
    """ScanContext represents the DVR-Scan program state, including the current settings for
    various options, coordinating overall scanning logic (via the scan_motion() method)."""
    ...
    def __init__(self,
        ...
        self._detector_type = DetectorType.MOG2 # -b/--bg-subtractor
    ...
    def set_detection_params(self,
                             ...
                             roi_list: Optional[List[Tuple[int, int, int, int]]] = None,
        ...
        assert roi_list is None or all(all(isinstance(x, int) for x in roi) for roi in roi_list)
    ...
    def _select_roi(self) -> bool:
        ...
        if self._roi_list:
            self._logger.info('Detection regions [X0 Y0, W H]: %s',
    ...
    def scan_motion(self) -> Optional[ScanResult]:
        """ Performs motion analysis on the ScanContext's input video(s). """
        self._stop.clear()
        buffered_frames: List[numpy.ndarray] = []
        ...
        if self._bounding_box:
            if len(self._roi_list) > 1:
                # TODO(v1.6): Support multiple ROI windows.
                raise NotImplementedError("Only one ROI is currently supported.")
            self._bounding_box.set_corrections(
                downscale_factor=self._downscale_factor,
                roi=self._roi_list[0] if self._roi_list else None,
        ...
        with context_manager:
            ...
            while not self._stop.is_set():
                ...
                frame.frame_rgb = motion_regions.crop_and_scale(frame.frame_rgb)
                ...
                pixels_of_interest = motion_regions.mask(motion_mask)
                frame_score = cv2.sumElems(motion_mask)[0] / float(pixels_of_interest)
        ...
        return ScanResult(event_list, frames_processed)
